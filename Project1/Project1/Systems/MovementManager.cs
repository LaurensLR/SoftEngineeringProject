    ...
    */
    public class MovementManager
    {
        ...
        private float CalculateHorizontalAllowance(IMovable movable, float desiredMoveX, List<IGameObject> worldObjects)
        {
            ...
            // BOUNDARY CHECK: Clamp to map edges
            if (futureRect.Left < 0)
            {
                moveX = Math.Max(moveX, -movable.Position.X); // Stop at left edge (0)
            }
            else if (futureRect.Right > MapWidth)
            {
                moveX = Math.Min(moveX, MapWidth - movable.Position.X - movable.Width); // Stop at right edge (800)
            }
